...
- Fixed possible endless loop if connection to broker fails while waiting for messages from subscribed topics (if `WAIT_FOR_SUBSCRIPTIONS` is defined in `platformio.ini`)
